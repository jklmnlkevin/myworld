...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			...
			<sec:authorize ifAnyGranted="payRecord.delete">
	...
	<table class="table" width="100%" layoutH="138" nowrapTD="false">
		<thead>
			<tr>
				...
				<th align="center" width="100">缴费时间</th>
				<th align="center" width="100">缴费项目</th>
				<th align="center" width="100">缴费月数</th>
				<th align="center" width="100">缴费开始月份</th>
				<th align="center" width="100">缴费结束月份</th>
				<th align="center" width="100">支付类型</th>
		...
		<tbody>
			...
			<tr target="sid_user" rel="${n.id }">
				...
				<td>
				<fmt:formatDate value="${n.payTime}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					<c:forEach items="${feeItemTypes}" var="i">
							<c:if test="${i.value eq n.feeItem.type }">${i.remark }
								<c:set var="typeName" value="${i.remark }"/>
							</c:if>
						</c:forEach>
				</td>
				<td>${n.monthCount}</td>
				<td>
								<fmt:formatDate value="${n.monthStart}" pattern="yyyy-MM"/>
				<td>
				<fmt:formatDate value="${n.monthEnd}" pattern="yyyy-MM"/>
				<td>
						<c:forEach items="${payTypes }" var="t">
							${t.value eq n.payType ? t.remark : ''}
						</c:forEach>
				</td>
				<%-- <td>${n.createTime}</td> --%>
				<td>${n.money}</td>
