 ...
 */
// @Module(name = "系统消息管理模块")
@Controller
@RequestMapping(value = "/admin/systemMessage", produces="text/html;charset=UTF-8") // produces是为了解决@ResponseBody返回乱码
public class AdminSystemMessageController extends BaseController {
	...
	public String detail(Map<String, Object> map, Long id) {
		if (id != null) {
			SystemMessageDTO dto = systemMessageService.load(id);
			map.put("n", dto);
		}
		return "admin/systemMessage/systemMessage_detail";
	}
	 ...
	 * @param dto
	 */
	@Log(operation = "保存系统消息管理") 
	@ResponseBody
	@RequestMapping(value = "/save")
	@PreAuthorize("(hasRole('systemMessage.update') and #dto.id != null) or (hasRole('systemMessage.add') and #dto.id == null)")
	public String save(SystemMessageDTO dto, JsonResultDTO resultDTO) throws Exception {
		if (dto.getId() == null) {
			systemMessageService.create(dto);
		} else {
			systemMessageService.updateAllFields(dto);
		}
		return okAndClose(resultDTO);
	}

    @Log(operation = "删除系统消息管理")
	...
	public String list(SystemMessageDTO dto, Map<String, Object> map, Page page) {
		List<SystemMessageDTO> dtos = systemMessageService.find(dto, page);
		// 这个数据是用来展示的
		map.put("list", dtos);
		// 这个数据是保存查询条件的
		map.put("dto", dto);
		return "admin/systemMessage/systemMessage_list";
	}
	...
	public String search(SystemMessageDTO dto, Map<String, Object> map, Page page) {
		List<SystemMessageDTO> dtos = systemMessageService.find(dto, page);
		// 这个数据是用来展示的
		map.put("list", dtos);
		// 这个数据是保存查询条件的
		map.put("dto", dto);
		return "admin/systemMessage/systemMessage_search";
	}
    ...
    public String push(Map<String, Object> map, Long id) {
        if (id != null) {
            SystemMessageDTO dto = systemMessageService.load(id);
            map.put("systemMessage", dto);
        }
        return "admin/systemMessage/systemMessage_push";
    }
    
    @ResponseBody
    @RequestMapping(value = "/doPush")
    public String doPush(Map<String, Object> map, Long id, JsonResultDTO resultDTO) {
        SystemMessageDTO systemMessage = systemMessageService.load(id);
        systemMessageService.push(systemMessage);
        return okAndClose(resultDTO);
    }
}
