...
// @Module(name = "楼栋号模块")
@Controller
@RequestMapping(value = "/admin/building", produces="text/html;charset=UTF-8") // produces是为了解决@ResponseBody返回乱码
public class AdminBuildingController extends BaseController {
	...
	public String detail(Map<String, Object> map, Long id) {
		if (id != null) {
			BuildingDTO dto = buildingService.load(id);
			map.put("n", dto);
		}
		return "admin/building/building_detail";
	}
	...
	public String save(BuildingDTO dto, JsonResultDTO resultDTO) throws Exception {
		if (dto.getId() == null) {
			buildingService.create(dto);
		} else {
			buildingService.updateAllFields(dto);
		}
	...
	public String list(BuildingDTO dto, Map<String, Object> map, Page page) {
		List<BuildingDTO> dtos = buildingService.find(dto, page);
		// 这个数据是用来展示的
		map.put("list", dtos);
		// 这个数据是保存查询条件的
		map.put("dto", dto);
		return "admin/building/building_list";
	}
	...
	public String search(BuildingDTO dto, Map<String, Object> map, Page page) {
		List<BuildingDTO> dtos = buildingService.find(dto, page);
		// 这个数据是用来展示的
		map.put("list", dtos);
		// 这个数据是保存查询条件的
		map.put("dto", dto);
		return "admin/building/building_search";
	}
}
