 ...
 */
@Repository
public class BuildingDAO extends GenericDAOHibernate<Building> {
     ...
     * @return
     */
    public List<Building> find(BuildingDTO dto, Page page) {
        StringBuilder hql = new StringBuilder();
        List<Object> paras = new ArrayList<Object>();
        hql.append("From Building n where 1 = 1 ");
        if (dto.getId() != null) {
            hql.append(" and n.id = ? ");
            paras.add(dto.getId());
        }
        ...
        if (dto.getCommunity() != null) {
        	...
        	if (StringUtils.isNotBlank(dto.getCommunity().getName())) {
        	    hql.append(" and n.community.name like ? ");
        	    paras.add("%" + dto.getCommunity().getName() + "%");
        	}
        }			
     ...
     * @return
     */
    public Building findOne(BuildingDTO dto) {
		Page page = new Page();
		page.setNumPerPage(1);
		page.setPageNum(1);
		
		List<Building> list = this.find(dto, page);
