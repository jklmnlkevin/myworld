	...
	<table class="table" width="100%" layoutH="138" nowrapTD="false">
		<thead>
			<tr>
				...
				<th align="center" width="100">类别名称</th>
				<th align="center" width="100">查找带回</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${categorys }" var="n">
			<tr target="sid_category" rel="${n.id }">
				<td><input name="ids" value="${n.id }" type="checkbox"></td>
					<td>${n.name}</td>
				<td>
					<a class="btnSelect" href="javascript:$.bringBack({name:'${n.name }', id:'${n.id }', orgNum:'1001'})" title="查找带回">选择</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
