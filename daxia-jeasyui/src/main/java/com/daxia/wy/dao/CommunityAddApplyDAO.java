 ...
 */
@Repository
public class CommunityAddApplyDAO extends GenericDAOHibernate<CommunityAddApply> {
     ...
     * @return
     */
    public List<CommunityAddApply> find(CommunityAddApplyDTO dto, Page page) {
        StringBuilder hql = new StringBuilder();
        List<Object> paras = new ArrayList<Object>();
        hql.append("From CommunityAddApply n where 1 = 1 ");
        if (dto.getId() != null) {
            hql.append(" and n.id = ? ");
            paras.add(dto.getId());
        }
     ...
     * @return
     */
    public CommunityAddApply findOne(CommunityAddApplyDTO dto) {
		Page page = new Page();
		page.setNumPerPage(1);
		page.setPageNum(1);
		
		List<CommunityAddApply> list = this.find(dto, page);
