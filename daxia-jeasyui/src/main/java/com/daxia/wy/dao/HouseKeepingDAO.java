 ...
 */
@Repository
public class HouseKeepingDAO extends GenericDAOHibernate<HouseKeeping> {
     ...
     * @return
     */
    public List<HouseKeeping> find(HouseKeeping dto, Page page) {
        StringBuilder hql = new StringBuilder();
        List<Object> paras = new ArrayList<Object>();
        hql.append("From HouseKeeping n where 1 = 1 ");
        ...
        if (dto.getServiceType() != null) {
            if (dto.getServiceType().getId() != null) {
                hql.append(" and n.serviceType.id = ? ");
                paras.add(dto.getServiceType().getId());
            }
        }
        if (dto.getType() != null) {
     ...
     * @return
     */
    public HouseKeeping findOne(HouseKeeping dto) {
		Page page = new Page();
		page.setNumPerPage(1);
		page.setPageNum(1);
		
		List<HouseKeeping> list = this.find(dto, page);
	    return CollectionUtils.isEmpty(list) ? null : list.get(0);
    }
    
    @Override
    public Long create(HouseKeeping t) {
