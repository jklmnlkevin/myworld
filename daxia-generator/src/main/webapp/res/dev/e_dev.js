var modelPackage = "com.daxia.generator";
...
/*		// 验证父级菜单
		$("input[name=parentMenu]").autocomplete({
		     source: function(request, response) {
		    	  $.ajax({
		              url: "${ctx}/dev/findParentMenu",
		              dataType: "json",
		              data: {
		            	  level: 2,
		            	  term: request.term
		...
		$("input[name=parentAuthority]").autocomplete({
		     source: function(request, response) {
		    	  $.ajax({
		              url: "${ctx}/dev/findParentAuthority",
		              dataType: "json",
		              data: {
		            	  level: 2,
		            	  term: request.term
		...
		$("input[name=parentAuthorityCode]").autocomplete({
		     source: function(request, response) {
		    	  $.ajax({
		              url: "${ctx}/dev/findParentAuthority",
		              dataType: "json",
...
*/		
		...
		$.ajax({
            ...
            success: function(data) {
            	...
            	for (var i = 0; i < data.length; i++) {
            		...
            		newTr.find('input[field=name]').val(row.name);
	...
	function convertType(type, column) {
		...
		realType = modelPackage + "." + realType;
	...
	function generate() {
		...
		document.cookie = 'e.projectPath=' + params.projectPath + "; expires=Thu, 18 Dec 2020 12:00:00 GMT";
		document.cookie = 'e.basePackage=' + params.basePackage + "; expires=Thu, 18 Dec 2020 12:00:00 GMT";
